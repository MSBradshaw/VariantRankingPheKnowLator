...
"""
The input data for the above script requires the graph to be formatted correctly (nodea nodeb) 
rather than nodea edge nodeb like most programs output. 
The code chunk below reads in a text file containing edges lists formatted as triples and outputs a modified edge list 
with the edges removed (i.e. nodea nodeb).
"""

file_loc = 'Embeddings/PheKnowLator_Subclass_InverseRelations_NotClosed_NoOWL_Triples_Integers.txt'

with open(file_loc) as f_in:
    kg_data = [x.split('\t')[0::2] for x in f_in.read().splitlines()]

f_in.close()

# write out cleaned file
file_out = 'Embeddings/PheKnowLator_Subclass_InverseRelations_NotClosed_NoOWL_Integers-NOT-TRIPLE.txt'
