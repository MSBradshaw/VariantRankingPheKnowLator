...
paths = []
if True or not os.path.exists('AA_TOF_node_counts.pickle') or not os.path.exists('AA_TOF_counts.pickle') or not os.path.exists('AA_TOF_path_lengths.pickle'):
    counts = {}
    node_counts = {}
        ...
        for target in targets:
            ...
            for node in path:
                if node in node_counts:
                    ...
                    node_counts[node] = 1
    pickle.dump(counts, open('AA_TOF_counts.pickle', 'wb'))
    pickle.dump(node_counts, open('AA_TOF_node_counts.pickle', 'wb'))
    pickle.dump(path_lengths, open('AA_TOF_path_lengths.pickle', 'wb'))

counts = pickle.load(open('AA_TOF_counts.pickle', 'rb'))
node_counts = pickle.load(open('AA_TOF_node_counts.pickle', 'rb'))
path_lengths = pickle.load(open('AA_TOF_path_lengths.pickle', 'rb'))
...
plt.savefig('Figures/AA_TOF_predicate_counts_log.png')
plt.show()
...
plt.ylabel('Count')
plt.xlabel('Predicate')
plt.savefig('Figures/AA_TOF_predicate_counts.png')
plt.show()

# do it again with shuffled targets

shuffled_paths = []
if True or not os.path.exists('shuffled_AA_TOF_node_counts.pickle') or not os.path.exists('shuffled_AA_TOF_counts.pickle') or not os.path.exists('shuffled_AA_TOF_path_lengths.pickle'):
    shuffled_counts = {}
    ...
    nodes = partition(disease_genes, 2)
    seeds, targets = nodes[0], nodes[1]
    shuffled_nodes = partition(disease_gene_sets['TETRALOGY OF FALLOT'], 2)
        ...
        for target in shuffled_targets:
            ...
            for node in path:
                if node in shuffled_node_counts:
                    ...
                    shuffled_node_counts[node] = 1
    pickle.dump(shuffled_counts, open('shuffled_AA_TOF_counts.pickle', 'wb'))
    pickle.dump(shuffled_node_counts, open('shuffled_AA_TOF_node_counts.pickle', 'wb'))
    pickle.dump(shuffled_path_lengths, open('shuffled_AA_TOF_path_lengths.pickle', 'wb'))

shuffled_counts = pickle.load(open('shuffled_AA_TOF_counts.pickle', 'rb'))
shuffled_node_counts = pickle.load(open('shuffled_AA_TOF_node_counts.pickle', 'rb'))
shuffled_path_lengths = pickle.load(open('shuffled_AA_TOF_path_lengths.pickle', 'rb'))
...
g = sns.barplot(shuffled_df['predicate'], shuffled_df['count'])
g.set_yscale("log")
plt.xticks(rotation=90)
plt.tight_layout()
plt.ylabel('Count')
plt.xlabel('Predicate')
plt.savefig('Figures/shuffled_AA_TOF_predicate_counts_log.png')
plt.show()
...
plt.ylabel('Count')
plt.xlabel('Predicate')
plt.savefig('Figures/shuffled_AA_TOF_predicate_counts.png')
plt.show()

# plot change
# get all predicates
all_preds = set(df['predicate']).union(set(shuffled_df['predicate']))
diff_dict = {'predicate': [], 'difference': [], 'normal':[]}
for p in all_preds:
    norm_count = 0
    shuf_count = 0
    if p in counts:
